...
> 시간순 행동 + 검증 증거 기록. 결정/규칙은 `CLAUDE.md`, 설계·실험순서는 `README.md §실험 순서` 참조.
> *아래 로그 본문은 당시 행동 기록이라 그때 존재한 파일명(EXPERIMENT_PLAN.md·DEBUGGING.md·병렬화-KV전송-측정.md·trtllm_support_matrix.md·disagg_config.yaml)을 그대로 둔다. 이 문서들은 2026-06-23 4종으로 통합됨 → 현 위치는 `CLAUDE.md §문서 인덱스` 참조.*
