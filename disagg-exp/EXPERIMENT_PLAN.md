...
> 사용자 방침: **코드는 내가 작성하지 않음.** 사용자가 연구실 공식 Org에 fork하여 직접 구현. 이 문서는 그 작업의 설계·런북. 모든 런타임은 원격 EC2에서 SSH로(`aws` skill).
...
- **2-phase**: warmup(10~50) → 건강검진(`fail_rate>0.30` or `ttft_p99>300s` → measured 스킵 + `.failed` 마커) → measured **300**(p99 신뢰도). `.done/.failed` resume.
